...
CREATE   PROCEDURE [dbo].[StdNaming]
    @PreviewOnly BIT = 1,
...
/*
    ...
    (
        SELECT
            ...
            LEFT('FK_' + OBJECT_NAME(fk.parent_object_id) + '_' + OBJECT_NAME(fk.referenced_object_id), 128) AS proposed_name
    ...
    (
        SELECT
            ...
            LEFT('CK_' + OBJECT_NAME(cc.parent_object_id) + '_' + CAST(cc.object_id AS VARCHAR(20)), 128) AS proposed_name
        FROM sys.check_constraints AS cc
        WHERE OBJECT_NAME(cc.parent_object_id) <> 'sysdiagrams'
    ),
    df_names AS
    (
        SELECT
            ...
            LEFT('DF_' + OBJECT_NAME(dc.parent_object_id) + '_' + col.name, 128) AS proposed_name
        FROM sys.default_constraints AS dc
        INNER JOIN sys.columns AS col
            ON dc.parent_object_id = col.object_id
           AND dc.parent_column_id = col.column_id
        WHERE OBJECT_NAME(dc.parent_object_id) <> 'sysdiagrams'
    ),
    key_names AS
    (
        SELECT
            ...
            LEFT(
                CASE
                    WHEN kc.type = 'PK' THEN 'PK_' + OBJECT_NAME(kc.parent_object_id)
                    WHEN kc.type = 'UQ' THEN 'UQ_' + OBJECT_NAME(kc.parent_object_id) + '_' + ISNULL(puc.column_list, 'COL')
                END,
                128
            ) AS proposed_name,
    ...
    (
        ...
        GROUP BY i.object_id, i.index_id, s.name, t.name, i.name
    )
    ...
    INNER JOIN
    (
        SELECT schema_name, proposed_name
        FROM #Candidates
        WHERE rename_command IS NOT NULL
        GROUP BY schema_name, proposed_name
        HAVING COUNT(*) > 1
    ) AS d
    ...
    SELECT
        ...
        rename_command,
        CASE
            WHEN @PreviewOnly = 1 THEN validation_status
            WHEN validation_status = 'READY' THEN 'PENDING_EXECUTION'
    ...
    BEGIN
        BEGIN TRY
            EXEC sys.sp_executesql @rename_command;
            ...
            SET
                ...
                error_number = NULL,
                error_message = NULL,
                log_datetime = SYSDATETIME()
            WHERE run_id = @RunId
              AND object_type = @object_type
              AND schema_name = @schema_name
              AND ISNULL(table_name, '') = ISNULL(@table_name, '')
