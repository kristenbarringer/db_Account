...
(
    ...
    [customer_account_info_uuid] UNIQUEIDENTIFIER CONSTRAINT [df_customer_account_info_customer_account_info_uuid] DEFAULT (NEWSEQUENTIALID()) NOT NULL,   
    [customer_account_info_rid]  BIGINT IDENTITY (1, 1) NOT NULL,
    ...
    [is_active] BIT CONSTRAINT [df_customer_account_info_is_active] DEFAULT (1) NOT NULL,
    -- date columns (suffixed with "_date")
    [created_date] DATETIME CONSTRAINT [df_customer_account_info_created_date] DEFAULT (getdate()) NOT NULL,
...
ALTER TABLE [dbo].[customer_account_information] ADD CONSTRAINT [pk_customer_account_info_tenant_uuid_customer_account_info_rid] PRIMARY KEY CLUSTERED ([tenant_uuid] ASC, [customer_account_info_rid] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE);
GO
ALTER TABLE [dbo].[customer_account_information] ADD CONSTRAINT [uk_customer_account_info_uuid] UNIQUE NONCLUSTERED ([customer_account_info_uuid] ASC) WITH (FILLFACTOR = 100, DATA_COMPRESSION = PAGE);
GO
ALTER TABLE [dbo].[customer_account_information] ADD CONSTRAINT [uk_customer_account_info_rid] UNIQUE ([customer_account_info_rid]);
GO


-- fks - to tenant
ALTER TABLE [dbo].[customer_account_information] ADD CONSTRAINT [fk_customer_account_info_tenant_uuid] FOREIGN KEY ([tenant_uuid]) 
...
REFERENCES [dbo].account_details  ([tenant_uuid]); 
 GO
CREATE NONCLUSTERED INDEX [ix_fk_customer_account_info_tenant_uuid] 
  ON [dbo].[customer_account_information]([tenant_uuid] ASC); 
  GO
-- fks - to user
ALTER TABLE [dbo].[customer_account_information]
    ADD CONSTRAINT [fk_customer_account_info_created_by_user_rid] FOREIGN KEY ([created_by_user_rid]) 
/* On Fleet, should reference user_ref, on Account should reference user_accounts */
    REFERENCES [dbo].user_accounts ([user_rid]);
GO
CREATE NONCLUSTERED INDEX [ix_fk_customer_account_info_created_by_user_rid] 
    ON [dbo].[customer_account_information]([created_by_user_rid] ASC);
GO
ALTER TABLE [dbo].[customer_account_information]
    ADD CONSTRAINT [fk_customer_account_info_updated_by_user_rid] FOREIGN KEY ([updated_by_user_rid]) 
/* On Fleet, should reference user_ref, on Account should reference user_accounts */
    REFERENCES [dbo].user_accounts ([user_rid]);
GO
CREATE NONCLUSTERED INDEX [ix_fk_customer_account_info_updated_by_user_rid] 

    ON [dbo].[customer_account_information]([updated_by_user_rid] ASC);
GO
-- fks - other main fks
-- fks - to lookup code
-- ALTER TABLE [dbo].[customer_account_information] ADD CONSTRAINT [fk_customer_account_info_language_code] FOREIGN KEY ([language_code]) REFERENCES [dbo].[lookup_code] ([code]); 
--  GO
-- CREATE NONCLUSTERED INDEX [ix_fk_customer_account_info_language_code] 
